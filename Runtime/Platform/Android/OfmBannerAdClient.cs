...
{
    public class OfmBannerAdClient : AndroidJavaProxy, IOfmBannerAdClient
    {

        private Dictionary<string, AndroidJavaObject> bannerHelperMap = new Dictionary<string, AndroidJavaObject>();

        private  OfmBannerAdListener anyThinkListener;

        public OfmBannerAdClient() : base("com.ofm.unitybridge.banner.BannerListener")
        {
        ...
        public void loadBannerAd(string placementId, string mapJson, string customRulesJson)
        {
            ...
            if(!bannerHelperMap.ContainsKey(placementId))
            {
                AndroidJavaObject bannerHelper = new AndroidJavaObject(
                    "com.ofm.unitybridge.banner.BannerHelper", this);
                bannerHelper.Call("initBanner", placementId);
                bannerHelperMap.Add(placementId, bannerHelper);
                Debug.Log("OfmBannerAdClient : no exit helper ,create helper ");
            }

            try
            {
                Debug.Log("OfmBannerAdClient : loadBannerAd ");
                bannerHelperMap[placementId].Call("loadBannerAd", mapJson, customRulesJson);
            }
            catch (System.Exception e)
            {
                System.Console.WriteLine("Exception caught: {0}", e);
                Debug.Log ("OfmBannerAdClient :  error."+e.Message);
            }
        ...
        public string checkAdStatus(string placementId)
        {
            ...
            Debug.Log("OfmBannerAdClient : checkAdStatus....");
            try
            {
                if (bannerHelperMap.ContainsKey(placementId))
                {
        ...
        {
            Debug.Log("OfmBannerAdClient : showBannerAd by position" );
            //todo
            try
            {
                if (bannerHelperMap.ContainsKey(placementId))
        ...
        {
            Debug.Log("OfmBannerAdClient : showBannerAd " );
        ...
        public void cleanBannerAd(string placementId)
        {
            ...
            Debug.Log("OfmBannerAdClient : cleanBannerAd" );
        ...
        {
            Debug.Log("OfmBannerAdClient : hideBannerAd");
        ...
        //广告加载成功
        public void onBannerLoaded(string placementId, string callbackJson)
        {
            Debug.Log("onBannerLoaded...unity3d.");
            if(anyThinkListener != null){
                anyThinkListener.onAdLoad(placementId, new OfmCallbackInfo(callbackJson));
            } 
        }
        ...
        public void onBannerClicked(string placementId, string callbackJson)
        {
            ...
            if (anyThinkListener != null)
            {
                anyThinkListener.onAdClick(placementId, new OfmCallbackInfo(callbackJson));
            }
        }
        ...
        public void onBannerShow(string placementId, string callbackJson)
        {
            ...
            if (anyThinkListener != null)
            {
                anyThinkListener.onAdImpress(placementId, new OfmCallbackInfo(callbackJson));
            }
        }
        ...
        public void onBannerClose(string placementId, string callbackJson)
        {
            ...
            if (anyThinkListener != null)
            {
                anyThinkListener.onAdCloseButtonTapped(placementId, new OfmCallbackInfo(callbackJson));
            }
        }
        //广告关闭
        public void onBannerAutoRefreshed(string placementId, string callbackJson)
        {
            ...
            if (anyThinkListener != null)
            {
                anyThinkListener.onAdAutoRefresh(placementId, new OfmCallbackInfo(callbackJson));
            }
        }
        //广告自动刷新失败
