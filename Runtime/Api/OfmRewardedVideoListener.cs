...
{
    public interface OfmRewardedVideoListener
    {
		/***
		 * 加载广告成功（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */ 
        void onRewardedVideoAdLoaded(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 ...
		 */ 
        void onRewardedVideoAdPlayStart(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 * 视频播放结束（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */ 
        void onRewardedVideoAdPlayEnd(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 ...
		 */ 
        void onRewardedVideoAdPlayClosed(string placementId,bool isReward, OfmCallbackInfo callbackInfo);
		/***
		 * 视频点击（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */ 
        void onRewardedVideoAdPlayClicked(string placementId, OfmCallbackInfo callbackInfo);
        /**
         * 激励下发（注意：对于Android来说，所有回调方法均不在Unity的主线程）
         */
        void onReward(string placementId, OfmCallbackInfo callbackInfo);
