...
{
    public interface OfmBannerAdListener
    {
		/***
		 * 广告请求成功（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */
        void onAdLoad(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 ...
		 */ 
        void onAdImpress(string placementId, OfmCallbackInfo callbackInfo);
		/**
		 * 广告点击（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */ 
        void onAdClick(string placementId, OfmCallbackInfo callbackInfo);
		/**
		 * 广告自动刷新（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */
        void onAdAutoRefresh(string placementId, OfmCallbackInfo callbackInfo);
        /**
        ...
        *广告关闭；某些厂商不支持（注意：对于Android来说，所有回调方法均不在Unity的主线程）
        */
        void onAdCloseButtonTapped(string placementId, OfmCallbackInfo callbackInfo);
    }
}
