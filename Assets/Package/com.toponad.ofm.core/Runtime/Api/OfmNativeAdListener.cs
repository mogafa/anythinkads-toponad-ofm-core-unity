...
{
    public interface OfmNativeAdListener
    {
		/***
		 * 广告请求成功（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */
        void onAdLoaded(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 ...
		 */ 
        void onAdImpressed(string placementId, OfmCallbackInfo callbackInfo);
		/**
		 * 广告点击（注意：对于Android来说，所有回调方法均不在Unity的主线程）
		 */ 
        void onAdClicked(string placementId, OfmCallbackInfo callbackInfo);
		/***
		 ...
		 */
        void onAdCloseButtonClicked(string placementId, OfmCallbackInfo callbackInfo);
